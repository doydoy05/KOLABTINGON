...
const LOCAL_LLAMA_MODEL_PATH = process.env.LOCAL_LLAMA_MODEL_PATH || "";
const hasLocalLlamaModel = LOCAL_LLAMA_MODEL_PATH && existsSync(LOCAL_LLAMA_MODEL_PATH);
...
createServer(async (req, res) => {
  ...
  if (hasLocalLlamaModel) {
    console.log(`Using local Llama model at ${LOCAL_LLAMA_MODEL_PATH}`);
  } else if (HF_API_KEY) {
    console.log(`No local model found. Falling back to Hugging Face model ${MODEL}`);
  } else if (LOCAL_LLAMA_MODEL_PATH) {
    console.log(`Local model path configured but not found: ${LOCAL_LLAMA_MODEL_PATH}`);
    console.log("Set LOCAL_LLAMA_MODEL_PATH to a valid GGUF file, or configure HF_API_KEY.");
  } else {
    console.log("No local model configured and no HF_API_KEY set. Chat will return a warning message.");
  }
});
